                      ...
                      const EvalKey &ek)
{
    ...
    for (int i = 0; i <= bkP::domainP::k * bkP::domainP::n; i++)
        temp1[i] = cs[i] + c1[i];
    for (int i = 0; i <= bkP::domainP::k * bkP::domainP::n; i++)
        temp0[i] = -cs[i] + c0[i];
    temp1[bkP::domainP::k * bkP::domainP::n] -= bkP::domainP::μ;
    temp0[bkP::domainP::k * bkP::domainP::n] -= bkP::domainP::μ;
                ...
                const EvalKey &ek)
{
    ...
    TLWE<typename bkP::domainP> tlwe1, tlwe0;
    IdentityKeySwitch<iksP>(tlwe1, temp1, ek.getiksk<iksP>());
    IdentityKeySwitch<iksP>(tlwe0, temp0, ek.getiksk<iksP>());
    HomMUXwoIKSandSE<bkP>(res, tlwe1, tlwe1, tlwe0, ek);
}
