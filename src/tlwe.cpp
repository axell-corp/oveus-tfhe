...
#define INST(P)                                               \
    template void tlweSymIntEncrypt<P, P::plain_modulus>(     \
        TLWE<P> & res, const typename P::T p, const Key<P> &key)
INST(lvl0param);
INST(lvl1param);
INST(lvl2param);
#undef INST
...
#define INST(P)                                                   \
    template typename P::T tlweSymIntDecrypt<P, P::plain_modulus>( \
        const TLWE<P> &c, const Key<P> &key)
INST(lvl0param);
INST(lvl1param);
INST(lvl2param);
#undef INST
