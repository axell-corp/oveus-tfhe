...
typename P::T tlweSymIntDecrypt(const TLWE<P> &c, const Key<P> &key)
{
    const double Δ =
        std::pow(2.0, std::numeric_limits<typename P::T>::digits) /
        plain_modulus;
    return static_cast<typename P::T>(std::round(tlweSymPhase<P>(c, key) / Δ)) %
           plain_modulus;
}
...
#pragma omp parallel for
    for (int i = 0; i < p.size(); i++)
        tlweSymEncrypt<P>(c[i], p[i] ? P::μ : -P::μ, key);
}
...
{
    for (int i = 0; i <= P::k * P::n; i++) res[i] = (first[i] + ... + rest[i]);
}
